...
func TestSlackService_ParsePRDirectives(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: &PRDirectives{HasReviewDirective: true,
				UsersToCC: []string{"john.doe-smith"},
			},
		},
		{
			name:        "Multiple CC users",
			description: "!review: @john.doe @jane.smith @team.lead",
			expected: &PRDirectives{HasReviewDirective: true,
				UsersToCC: []string{"john.doe", "jane.smith", "team.lead"},
			},
		},
		{
			name:        "Multiple CC users with channel",
			description: "!review: #dev-team @john.doe @jane.smith",
			expected: &PRDirectives{HasReviewDirective: true,
				Channel:   "dev-team",
				UsersToCC: []string{"john.doe", "jane.smith"},
			},
		},
		{
			name:        "Duplicate users (should deduplicate)",
			description: "!review: @john.doe @jane.smith @john.doe",
			expected: &PRDirectives{HasReviewDirective: true,
				UsersToCC: []string{"john.doe", "jane.smith"},
			},
		},
